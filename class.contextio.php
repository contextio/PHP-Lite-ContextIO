...
class ContextIO {
	...
	public function modifyWebhook($user, array $params) {
		...
		$params = $this->_filterParams($params, array('include_body','filter_to', 'filter_from', 'filter_cc', 'filter_subject', 'filter_thread', 'filter_file_name', 'callback_url', 'filter_folder_added','filter_folder_removed','filter_to_domain','filter_from_domain', 'body_type', 'include_header', 'receive_drafts', 'receive_all_changes'), array('webhook_id'));
		if ($params === false) {
			throw new InvalidArgumentException("params array contains invalid parameters or misses required parameters");
		}
